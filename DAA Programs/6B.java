import java.util.Scanner; class KObject {
float w; float p; float r;
}
public class KnapsackGreedy {
static final int MAX = 20; static int n;
static float M;
// Knapsack object details
// max. no. of objects // no. of objects
...
float totalprofit;
int i;
float U; // U place holder for M U = M;
totalprofit = 0;
for (i = 0; i < n; i++)
x[i] = 0;
for (i = 0; i < n; i++) {
...
totalprofit = totalprofit + (x[i] * kobj[i].p); 
  System.out.println("The Solution vector, x[]: "); for (i = 0; i < n; i++)
